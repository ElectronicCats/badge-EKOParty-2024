...
static void module_validate_code(){
  ...
  if(code_selected[0] == code_gato_1[0] && code_selected[1] == code_gato_1[1] && code_selected[2] == code_gato_1[2] && code_selected[3] == code_gato_1[3]){
    oled_screen_clear();
  ...
  }else if(code_selected[0] == code_gato_2[0] && code_selected[1] == code_gato_2[1] && code_selected[2] == code_gato_2[2] && code_selected[3] == code_gato_2[3]){
    oled_screen_clear();
    oled_screen_display_bitmap(llamaneitor_1, 0, 0, 32, 32, OLED_DISPLAY_NORMAL);
    oled_screen_display_text("Gato", 40, 1, OLED_DISPLAY_NORMAL);
  ...
  }else if(code_selected[0] == code_gato_3[0] && code_selected[1] == code_gato_3[1] && code_selected[2] == code_gato_3[2] && code_selected[3] == code_gato_3[3]){
    oled_screen_clear();
    oled_screen_display_bitmap(llamaneitor_1, 0, 0, 32, 32, OLED_DISPLAY_NORMAL);
    oled_screen_display_text("Gato", 40, 1, OLED_DISPLAY_NORMAL);
...
static void module_display_code_selector(){
  ...
  oled_screen_display_bitmap(simple_up_arrow_bmp, 0, 8*2, 8, 8, OLED_DISPLAY_NORMAL);
  oled_screen_display_bitmap(simple_down_arrow_bmp, 0, 8*3, 8, 8, OLED_DISPLAY_NORMAL);
  uint8_t x_post_offset = 8;
  for(uint8_t i = 0; i < 4; i++){
    snprintf(code, 4, "%d", code_selected[i]);
    oled_screen_display_text(code, (x_post_offset+16*i)+16, 2, (i == current_x_pos) ? OLED_DISPLAY_INVERT : OLED_DISPLAY_NORMAL);
  }
  oled_screen_display_text("OK", 100, 2, (current_x_pos == 4) ? OLED_DISPLAY_INVERT : OLED_DISPLAY_NORMAL);
}
