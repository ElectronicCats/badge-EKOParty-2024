...
static void module_display_character_selector(){
  ...
  oled_screen_display_bitmap(character_bitmap.bitmap, 32, 8, character_bitmap.width, character_bitmap.height, OLED_DISPLAY_NORMAL);
}
...
static void module_cb_event_main_menu(uint8_t button_name, uint8_t button_event){
  ...
  switch (button_name) {
    case BUTTON_UP:
      ...
      break;
    case BUTTON_DOWN:
      current_item = current_item < (MENU_COUNT - 1)
                       ? current_item + 1
                       : 0;
      general_screen_display_menu(current_item);
      break;
...
static void module_cb_event(uint8_t button_name, uint8_t button_event) {
  ...
  switch (button_name) {
    case BUTTON_UP:
    current_item = current_item > 0 ? current_item - 1 : (current_history.menu_count - 1);
    module_display_history();
      break;
    case BUTTON_DOWN:
